...
pub extern "C" fn js_segments_view_regexp_test(cursor: f64, regex: f64) -> f64 {
    ...
    let verdict = bounded_test_verdict(c, re, regex);
    match verdict {
        Some(v) => {
...
/// The engine-backed half of [`js_segments_view_regexp_test`]: `None` means
/// "decline: materialise and take the ordinary path". Without the
/// `regex-engine` feature (the compiler binary carries a runtime copy built
/// `default-features = false`) there is no engine to answer from, so every
/// call declines and the caller materialises as before.
#[cfg(feature = "regex-engine")]
fn bounded_test_verdict(
    c: *mut ObjectHeader,
...
fn bounded_test_verdict(
    _c: *mut ObjectHeader,
    _re: *const crate::regex::RegExpHeader,
    _regex: f64,
) -> Option<bool> {
    None
}
