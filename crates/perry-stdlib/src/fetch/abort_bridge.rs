...
pub(crate) enum SignalState {
    ...
    None,
    /// The signal was already aborted before the request started (carries its
    /// ptr) — the caller should reject up front with the signal's reason.
    AlreadyAborted(usize),
...
pub(crate) fn take_pending_signal_watch() -> SignalState {
    ...
    if perry_runtime::url::js_abort_signal_is_aborted(signal) != 0 {
        return SignalState::AlreadyAborted(signal as usize);
    }
...
pub(crate) fn watch_or_reject(
    ...
    match state {
        SignalState::AlreadyAborted(signal_ptr) => {
            queue_deferred_resolution(promise_ptr, false, move || abort_reason_bits(signal_ptr));
            None
        }
...
/// request) and the promise rejects with the signal's reason (a `TimeoutError`
/// for `AbortSignal.timeout`, the controller's reason for `controller.abort()`).
/// The `watch` deregisters when dropped at the end of this scope.
pub(crate) async fn race_request<F: std::future::Future<Output = ()>>(
    ...
    match abort_watch {
        Some(watch) => {
            let signal_ptr = watch.signal_ptr;
            tokio::select! {
                _ = request_future => {}
                _ = watch.aborted() => {
                    queue_deferred_resolution(promise_ptr, false, move || abort_reason_bits(signal_ptr));
...
/// NaN-boxed bits of the aborted signal's `reason` for rejecting an aborted
/// fetch — a `TimeoutError` for `AbortSignal.timeout`, the controller's reason
/// for `controller.abort(reason)`, or a fresh `AbortError` fallback. Matches
/// Node, so retry logic that distinguishes timeout-vs-abort behaves correctly
/// (rejecting everything as `AbortError` turns a timeout into a retry spin).
/// Read inside a `queue_deferred_resolution` converter so the signal's reason is
/// touched on the main thread (never from a worker).
fn abort_reason_bits(signal_ptr: usize) -> u64 {
    perry_runtime::url::js_abort_signal_reason_bits(signal_ptr as i64)
}
