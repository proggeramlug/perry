...
The caller must hold the campaign lock. No sampling or entry/return probes.
"""
import argparse
from phase_recorder import PhaseRecorder as BpfCensus
import fcntl
import hashlib
...
def require_started_app(observed, pre_exec, app, app_argv, post_start_argv=None):
    expected = dict(pre_exec, exe=str(Path(app).resolve()))
    expected.pop("argv")
    require_identity(observed, expected)
    alternatives = [app_argv]
    if post_start_argv is not None:
        alternatives.append(post_start_argv)
    if observed["argv"] not in alternatives:
        raise RuntimeError(f"owned process {observed['pid']} has undeclared argv transition: {observed['argv']!r}")
    return observed
...
def stop_owned_app(pid, pre_exec, app, app_argv=None, post_start_argv=None):
    app_argv = [str(app)] if app_argv is None else app_argv
    ...
    alternatives = [pre_exec, dict(pre_exec, exe=str(app), argv=app_argv)]
    if post_start_argv is not None:
        alternatives.append(dict(pre_exec, exe=str(app), argv=post_start_argv))
...
def run(app, label, port, out, app_args=(), post_start_argv=None):
    app_argv = [str(app), *map(str, app_args)]
    ...
    receipt = {
        "label": label, "application": str(app), "application_sha256": sha(app), "application_argv": app_argv,
        "fixture_sha256": fixtures, "command": PROMPT,
        "answer_characters": len(ANSWER), "instrumentation": "controller phase timestamps only; no perf/BPF or runtime counters",
        "phases": {}, "process_cpu": {}, "pass": False,
    }
    cleanup_errors = []
    try:
        ...
        if pid == 0:
            try:
                ...
                os.execvpe(str(app), app_argv, env)
            finally:
                os._exit(127)
        receipt["pid"] = pid
        ...
        perf.phase(2)
        receipt["process_cpu"]["startup_s"] = driver.cpu_of(pid)
        observed = process_snapshot(pid)
        receipt["observed_startup_process"] = observed
        receipt["process"] = require_started_app(observed, pre_exec, app, app_argv, post_start_argv)
        ...
        perf.phase(4)
        receipt["process_cpu"]["command_s"] = driver.cpu_of(pid) - cpu_before
        receipt["vmhwm_mb"] = driver.peak_mb(pid)
        receipt["counter_control"] = perf.stop()
        perf = None
        ...
        if pid is not None:
            try:
                ...
                if pre_exec is not None:
                    receipt["app_cleanup"] = stop_owned_app(pid, pre_exec, app, app_argv, post_start_argv)
                else:
                    try:
                        ended, status = os.waitpid(pid, os.WNOHANG)
