...
def main():
    ...
    status = base / 'census-observation-v2.execution.json'
    assert not status.exists()
    lock = Path('/root/rig9831/lock')
    owner = f'canonical-index-census-rows-20260912 {os.getpid()}\n'
    try:
        lock.mkdir()
    ...
    record = {'status': 'started', 'pid': os.getpid(), 'owner': owner.strip(),
              ...
              'inputs': {name: sha(base / name) for name in
                         ['census_workload_v1.py', 'counter_observer.py', 'bindings-v2.json',
                          'run_census_v2.py']}}
    save(status, record)
    try:
        for row in (1, 2, 3):
            out = base / f'census-row-v2-{row}'
            result = run(app, f'canon-census-v2-{row}', 10781 + row, out)
            ...
            print(json.dumps({'row': row, 'calls': final['CALLS'],
                              ...
                              'diag': (out / 'canonical-diag.txt').read_text().strip()[-220:],
