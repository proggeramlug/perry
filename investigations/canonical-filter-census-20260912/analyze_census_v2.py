...
FILTERS = ['SYMBOL_ADDR_FILTER', 'CLASS_PROTOTYPE_ADDR_FILTER',
           'BUFFER_LIKE_ADDR_FILTER']
COUNTERS = ['CALLS', 'FILTER_PASS', 'RESOLVED', 'ADMITS', 'RETIRES']


def main():
    base = Path(__file__).resolve().parent
    record = json.loads((base / 'census-observation-v2.execution.json').read_text())
    ...
    for name in ('startup', 'command'):
        ...
        totals[name] = {'CALLS': calls, 'FILTER_PASS': passes, 'RESOLVED': resolved,
                        ...
                        'resolved_share_of_passes': (resolved / passes) if passes else None}
    out['totals'] = totals
    (base / 'census-analysis-v2.json').write_text(json.dumps(out, indent=1) + '\n')
