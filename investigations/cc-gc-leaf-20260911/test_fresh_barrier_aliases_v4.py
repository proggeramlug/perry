...
import fresh_barrier_census_v4 as c
...
class AliasTests(unittest.TestCase):
    def test_identical_diamond_origins_survive_but_collecting_arm_does_not(self):
        b = '''%slot = alloca i64, align 8
%a = call i64 @js_object_alloc(i32 0, i32 2)
...
call void @js_write_barrier(i64 %loaded, i64 7)'''
        ...
        self.assertEqual(c.analyze(fn(b), local_aliases=True, intersect_joins=True)['strict_fresh_origin_sites'], 1)
        collecting = b.replace('right:\n', 'right:\ncall void @may_collect()\n')
        self.assertEqual(c.analyze(fn(collecting), local_aliases=True, intersect_joins=True)['strict_fresh_origin_sites'], 0)
        different = b.replace('right:\n', 'right:\n%other = call i64 @js_array_alloc(i32 1)\nstore i64 %other, ptr %slot, align 8\n')
        self.assertEqual(c.analyze(fn(different), local_aliases=True, intersect_joins=True)['strict_fresh_origin_sites'], 0)

    def test_loop_intersection_does_not_manufacture_entry_fact(self):
...
br i1 true, label %loop, label %done
        ...
        r = c.analyze(fn(b), local_aliases=True, intersect_joins=True)
