...
FORMAT_STATUS = 'validated-installed-perf6.17-three-profile-v4-exports'
...
def validate_run(run):
    ...
    require(perf.get('exit') in (0, -2) and perf.get('errors') == [] and not perf.get('constructor_error'),
...
def load_callsites(directory, audit_sha):
    ...
    summary_path = directory / 'summary.json'
    summary = load_json(summary_path)
    require(summary.get('status') in ('complete-static-emission-census', 'complete-derived-leaf-coverage'), 'callsite census is incomplete')
    ...
    for row in csv_rows(directory / names[0], ('hypothesis', 'callee_identity', 'attempt', 'caller', 'statepoints', 'gc_live_operands')):
        require(row['hypothesis'] in ('direct-reviewed-runtime', 'new-local-user', 'new-unique-visible-user',
                                      'existing-proof-user', 'full-unique-visible-user', 'direct-known-cannot-collect', 'direct-all-runtime-leaves'), 'unknown hypothesis arm')
...
def analyze(samples_path, run_path, maps_path, output, metadata_dir=None, audit_path=None, callsite_dir=None):
    ...
    summary = {'schema': 1, 'status': 'complete-actual-ip-self-census', 'format': FORMAT,
               ...
               'limitations': [
                   'Installed perf6.17 exports and independent self reports are bound separately in measurement receipts.',
