...
def main():
    ...
    with (args.callsites / 'post-opt-hypothesis-targets.csv').open() as stream:
        for row in csv.DictReader(stream):
            if row['hypothesis'] == 'direct-reviewed-runtime' and int(row['statepoints']):
    ...
    result = {'status': 'complete-bounded-machine-region-coverage',
              ...
              'limitations': [
                  'Only20 hottest unique directly-reviewed-runtime candidate callers, exact PGCM symbol joins.',
