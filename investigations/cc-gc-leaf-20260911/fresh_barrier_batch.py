...
import fresh_barrier_census_v2 as barrier


def function_texts(lines, digest):
...
def analyze_one(text, start, extra, noncollecting):
    ...
    result = barrier.analyze(text, extra, noncollecting, local_aliases=True)
...
def run(captures, audit_path, output, llvm_dis, text_root=None):
    ...
    if len(extra) != 44:
        raise ValueError(f'expected the specifically authorized 44-summary population; got {len(extra)}')
    ...
    (output / 'extra-44.json').write_text(json.dumps(extra, indent=2) + '\n')
        ...
        for attempt in attempts:
            ...
            try:
                with emission.llvm_lines(bc, llvm_dis, text_root) as lines:
                    for start, end, text in function_texts(lines, ir_hash):
                        ...
                        if row['excluded']:
                            ...
                            for name in ('strict_barrier_sites', 'strict_fresh_origin_sites', 'extra_leaf_fresh_origin_sites',
                                         ...
                                         'narrow_extra_leaf_fresh_origin_sites'):
    ...
    result = {'schema': 1, 'status': 'partial-static-barrier-census' if partial else 'complete-static-barrier-census',
              ...
              'files': {p.name: {'sha256': emission.sha(p), 'bytes': p.stat().st_size}
                        for p in (rows_path, output / 'cannot-collect.json', output / 'extra-44.json')},
              'limits': ['generation and publication of fresh allocator returns remain unknown',
                         'all CFG joins and possibly collecting calls kill freshness',
