...
def inventory_records(lines):
    """Read unit metadata without parsing unrelated call operands twice.

    The subsequent effect/count passes still consume records() and validate
    every call. This pass needs only direct shadow-helper presence in bodies.
    """
    fn, pending, first_line = None, '', 0
    for lineno, raw in enumerate(lines, 1):
        stripped = raw.lstrip()
        if pending:
            line = uncomment(raw)
            ...
            pending += ' ' + line
            if call_complete(pending):
                yield ('call', fn, None, first_line, pending)
                pending = ''
            continue
        if fn is not None:
            if stripped.startswith('}'):
                line = uncomment(raw)
                if line == '}':
                    yield ('end', fn, None, lineno, line)
                    fn = None
            elif stripped.startswith('define '):
                raise ValueError(f'nested LLVM definition at line {lineno}')
            elif '@js_shadow_' in stripped:
                line = uncomment(raw)
                if OP.match(line):
                    if call_complete(line):
                        yield ('call', fn, None, lineno, line)
                    else:
                        pending, first_line = line, lineno
            continue
        if not stripped.startswith(('define ', 'declare ', 'attributes #', '@')):
            continue
        line = uncomment(raw)
        if line.startswith('define '):
            match = TARGET.search(line)
            if not match or not match[1].startswith('@') or not line.endswith('{'):
                raise ValueError(f'unsupported definition header at line {lineno}')
            fn = name(match[1])
            yield ('define', fn, None, lineno, line)
        elif line.startswith('declare '):
            yield ('declare', None, None, lineno, line)
        elif line.startswith('attributes #'):
            yield ('attributes', None, None, lineno, line)
            ...
            yield ('alias', None, None, lineno, line)
    if pending or fn is not None:
        raise ValueError('truncated LLVM function or shadow-helper call instruction')


def inventory(lines):
    result = {'definitions': {}, 'groups': {}, 'declarations': {}, 'aliases': {}, 'shadow': set()}
    for kind, fn, block, lineno, text in inventory_records(lines):
        if kind == 'define':
...
def count_unit(attempt):
    (inventories, pre_units, all_defs, audit, nonruntime, llvm_dis,
     text_root, visible, proof_sets, parts_dir) = WORKER_CONTEXT
    directory = attempt['directory']
    pre = inventories[(directory.name, STAGES[0])]
    per_callee, per_caller = defaultdict(Counter), defaultdict(Counter)
    categories, modes, totals = defaultdict(Counter), defaultdict(Counter), Counter()
    block_path = parts_dir / (directory.name + '.csv.gz')
    with gzip.open(block_path, 'wt', newline='', compresslevel=1) as stream:
    ...
    return (block_path, dict(per_callee), dict(per_caller), dict(categories), dict(modes), totals, found)
...
def analyze(root, audit_path, output, llvm_dis='llvm-dis', text_root=None, nonruntime_path=None):
    ...
    parts_dir = output / 'unit-parts'
    parts_dir.mkdir()
    WORKER_CONTEXT = (inventories, pre_units, all_defs, audit, nonruntime, llvm_dis,
                      text_root, visible, proof_sets, parts_dir)
    workers = int(os.environ.get('PERRY_CENSUS_WORKERS', '6'))
    if not 1 <= workers <= 6:
        raise ValueError('observer workers must be 1..6 under the campaign lock')
    # Fork preserves the already verified, read-only inventories and proof sets.
    # imap preserves original unit order; integer aggregation is unchanged.
    with multiprocessing.get_context('fork').Pool(workers) as pool:
        with gzip.open(block_path, 'wt', newline='') as f:
            ...
            for part, callee, caller, cats, rootmodes, unit_totals, found in pool.imap(count_unit, attempts):
                with gzip.open(part, 'rt', newline='') as stream:
                    shutil.copyfileobj(stream, f)
                for target, source in ((per_callee, callee), (per_caller, caller), (categories, cats),
                                       (modes, rootmodes), (opportunities, found)):
                    for key, value in source.items():
                        target[key].update(value)
                totals.update(unit_totals)
                print('COUNTED_UNIT', part.name, flush=True)
    WORKER_CONTEXT = None
