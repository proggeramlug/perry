...
SCALAR = r'(?:i64|double|ptr(?: addrspace\(\d+\))?)'
...
def exact_value_alias(op):
    ...
    for pattern in (
        ...
        r'ptrtoint ptr(?: addrspace\(\d+\))? (' + VALUE + r') to i64',
        r'inttoptr i64 (' + VALUE + r') to ptr(?: addrspace\(\d+\))?',
    ):
...
def analyze_function(fn, groups, declaration_leaves, extra_leaves, noncollecting, local_aliases=False):
    blocks = fn['blocks']
    if not blocks:
...
def analyze(ir, extra=(), noncollecting=(), local_aliases=False):
    functions, groups, declared = parse(ir)
    strict = [analyze_function(f, groups, declared, set(), set(noncollecting), local_aliases) for f in functions]
    hypothetical = [analyze_function(f, groups, declared, set(extra), set(noncollecting), local_aliases) for f in functions]
    def rows(result):
...
def main():
    ...
    result = analyze(args.ir.read_text(), extra, leaves, args.local_aliases)
