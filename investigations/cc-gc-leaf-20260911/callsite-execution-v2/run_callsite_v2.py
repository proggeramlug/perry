...
def main():
    ...
    while True:
        try:
            ...
            time.sleep(10)
    owner = f'cc-gcleaf-callsite-v2 {os.getpid()}'
    ...
    try:
        ...
        TEXT.mkdir()
        fixture_root = STAGE / 'fixture-ir-text-v1'
        fixture_root.mkdir()
        fixture_rows = []
        fixture_attempts, _ = census.read_attempts(STAGE / 'compile-v2/fixture-on-bitcode')
        for attempt in fixture_attempts:
            source = attempt['directory'] / 'pre-rs4gc.bc'
            dest = fixture_root / (attempt['directory'].name + '-pre-rs4gc.ll')
            command([str(LLVM), str(source), '-o', str(dest)], 'fixture-' + attempt['directory'].name)
            fixture_rows.append({'input': str(source), 'input_sha256': sha(source),
                                 'output': str(dest), 'output_sha256': sha(dest)})
        save('fixture-conversion.json', {'status': 'complete', 'files': fixture_rows})
        converted = []
        ...
        with (RUN / 'census.stdout').open('x') as stream:
            command(['python3', '-B', str(STAGE / 'callsite_census.py'), '--captures', str(CAPTURES),
                     '--helper-audit', str(STAGE / 'helper-audit-v2.csv'), '--output', str(OUTPUT),
