...
APP = STAGE / 'compile-v1/cc-diagnostic'
...
def main():
    ...
    while not (STAGE / 'compile-v1/COMPILE_COMPLETE.json').exists():
        if (STAGE / 'emission-v2-result.json').exists():
            result = json.loads((STAGE / 'emission-v2-result.json').read_text())
    ...
    while True:
        try:
            ...
            time.sleep(10)
    owner = f'cc-gcleaf-profile-v1 {os.getpid()}'
    (LOCK / 'owner').write_text(owner + '\n')
    try:
        for label, app in [('retained', STAGE.parent / 'cc-control/app-testdispatch25'),
                           ('diagnostic', APP)]:
        ...
        for number in range(1, 4):
            ...
            with (STAGE / (label + '-host-processes.txt')).open('x') as stream:
                subprocess.run(['ps', '-eo', 'pid,ppid,pcpu,etimes,args'], stdout=stream, check=True)
            ...
            with (out / 'samples.txt').open('x') as stream, (out / 'export.log').open('x') as log:
                subprocess.run(['perf', 'script', '--ns', '-i', str(out / 'profile.data'),
