...
def environment(label):
    ...
    home = Path("/tmp/tt_home_gcleaf_compile0911_v2_" + label)
...
def main():
    complete = json.loads((STAGE / "BUILD_COMPLETE-v4.json").read_text())
    ...
    while True:
        try:
            ...
            time.sleep(10)
    owner = f"cc-gcleaf-emission-v3 {os.getpid()}"
    ...
    try:
        ...
        receipt = json.loads((STAGE / "build-source-v3.json").read_text())
