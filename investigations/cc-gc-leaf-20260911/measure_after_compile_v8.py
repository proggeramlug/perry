...
def main():
    ...
    while True:
        try:
            ...
            time.sleep(10)
    owner = f'cc-gcleaf-profile-v8 {os.getpid()}'
    (LOCK / 'owner').write_text(owner + '\n')
    try:
        assert (STAGE / 'metadata-diagnostic/summary.json').exists()
        for number in range(2, 4):
            label = f'profile-v4-{number}'
            out = STAGE / label
            ...
            run(['python3', '-B', str(STAGE / 'profile_workload_v4.py'), '--app', str(APP),
        ...
        (STAGE / 'PROFILE_COMPLETE-v4.json').write_text(json.dumps({'profiles': 3, 'status': 'complete'}) + '\n')
