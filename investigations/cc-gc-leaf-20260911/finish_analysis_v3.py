...
R = S / 'final-analysis-execution-v3'
...
def command(label, args, acceptable=(0,)):
    ...
    env = dict(os.environ, PYTHONPATH=str(S / 'parser-v4'))
...
def main():
    ...
    while not (S / 'callsite-execution-v5/result.json').exists():
        assert not (S / 'callsite-execution-v5/failure.json').exists(), 'upstream failed'
    ...
    while True:
        try: L.mkdir(); break
            ...
            time.sleep(10)
    owner = f'cc-gcleaf-final-analysis-v3 {os.getpid()}'
    (L / 'owner').write_text(owner + '\n')
    try:
        inputs = json.loads((S / 'final-analysis-inputs-v3.json').read_text())
        for name, value in inputs.items(): assert sha(S / name) == value, name
        save('inputs.json', {'files': inputs, 'owner': owner, 'upstream_sha256': sha(S / 'callsite-execution-v5/result.json')})
        command('audit-join', [S/'join_actual_callees_v2.py', '--by-callee', S/'callsite-v5/by-callee.csv', '--audit', S/'helper-audit-v2.csv', '--output', S/'actual-audit-v1'])
        for i in range(1, 4):
            p = S / f'profile-v4-{i}'
            a = S / f'profile-analysis-v3-{i}'
            command(f'profile-join-{i}', [S/'profile_samples_v2.py', '--samples', p/'samples-v2.txt', '--run', p/'run.json', '--maps', p/'app-maps.json', '--metadata', S/'metadata-diagnostic', '--helper-audit', S/'helper-audit-v2.csv', '--callsites', S/'callsite-v5', '--output', a])
        ...
        command('machine-loop', [S/'run_machine_loop_coverage.py', '--elf', S/'compile-v2/cc-diagnostic', '--compile-receipt', S/'compile-v2/COMPILE_COMPLETE.json', '--metadata', S/'metadata-diagnostic', '--callsites', S/'callsite-v5', '--profile', S/'profile-analysis-v3-1', '--profile', S/'profile-analysis-v3-2', '--profile', S/'profile-analysis-v3-3', '--out', S/'machine-loop-v1'])
        command('barrier', [S/'fresh_barrier_parallel_v5.py', '--captures', S/'compile-v2/cc-bitcode', '--helper-audit', S/'helper-audit-v2.csv', '--text-root', S/'cc-ir-text-v1', '--output', S/'barrier-v5'], acceptable=(0, 2))
        barrier = json.loads((S/'barrier-v5/summary.json').read_text())
        census = json.loads((S/'callsite-v5/summary.json').read_text())
        ...
        save('result.json', {'status': 'complete', 'barrier_status': barrier['status'], 'barrier_sha256': sha(S/'barrier-v5/summary.json'), 'finished_ns': time.time_ns()})
    finally:
