...
RUN = STAGE / 'callsite-execution-v4'
OUT = STAGE / 'callsite-v4'
LOCK = Path('/root/rig9831/lock')
EXPECTED = {
 'parser-v3/callsite_census.py': '15d2b174d1a3ed738e06640e539c1ee21ca8c058ce53b403fe8bd79be832441c',
...
def main():
 ...
 while True:
  try:LOCK.mkdir();break
   ...
   time.sleep(10)
 owner=f'cc-gcleaf-callsite-v4 {os.getpid()}'
 ...
 try:
  ...
  command(['python3','-B',str(STAGE/'parser-v3/callsite_census.py'),
  ...
  command(['python3','-B',str(STAGE/'callsite_supplement_v1.py'),'--result',str(OUT),
           ...
           '--parser-dir',str(STAGE/'parser-v3'),'--output',str(OUT/'root-shadow-supplement.json')],'supplement')
