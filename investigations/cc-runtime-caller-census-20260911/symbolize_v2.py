...
def main():
    ...
    try:
        ...
        try:
            ...
            bindings = json.loads((base / 'bindings-v1.json').read_text())
            assert bindings['binary_sha256'] == target['binary_sha256']
            got = {r['slot']: r for r in bindings['relocations'] if r['is_counted_helper_relative_binding']}
            cached = json.loads((base / 'symbols-v1-1/summary.json').read_text())
            assert cached['binary_sha256'] == target['binary_sha256']
            cache = {r['start']: r for r in cached['owners']}
            result = []
            owners = {}
            ...
            for index, (start, function) in enumerate(sorted(owners.items())):
                ...
                previous_owner = cache.get(start)
                if previous_owner and previous_owner['size'] == function['size'] and previous_owner['aliases'] == function['aliases']:
                    path = base / 'symbols-v1-1' / previous_owner['disassembly']
                    assert sha(path) == previous_owner['sha256']
                else:
                ...
                function.update(disassembly=os.path.relpath(path, output), sha256=sha(path), command=command)
                ...
                for entry in result:
                    ...
                    slot = re.search(r'# ([0-9a-fA-F]+) <', operand)
                    if mnemonic in ('call', 'callq') and operand.startswith('*') and '(%rip)' in operand and slot and int(slot[1], 16) in got:
                        entry['call_binding'] = 'relative-GOT-call-to-counted-helper'
                        entry['got_binding'] = got[int(slot[1], 16)]
            record = {'status': 'complete-conservative-return-continuation-join', 'row': row,
                      ...
                      'script_sha256': sha(__file__), 'lock_owner': owner.strip(), 'got_bindings_sha256': sha(base / 'bindings-v1.json')}
