...
"""
import argparse
from filter_observer import FilterObserver as BpfCensus
import fcntl
import hashlib
...
def run(app, label, port, out):
    ...
    receipt = {
        ...
        "answer_characters": len(ANSWER), "instrumentation": "bounded read-only filter snapshots; no BPF or runtime counters",
        "phases": {}, "instrumented_cpu": {}, "pass": False,
    }
    cleanup_errors = []
    try:
        ...
        perf.phase(2)
        receipt["instrumented_cpu"]["startup_s"] = driver.cpu_of(pid)
        ...
        perf.phase(4)
        receipt["instrumented_cpu"]["command_s"] = driver.cpu_of(pid) - cpu_before
