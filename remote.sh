...
git push -q -f fork "$commit:refs/heads/lane/define-json-sync"
ssh -o BatchMode=yes root@perrymaster.skelpo.net "cd /root/claude-opencode/lanes/wt-define-json && git fetch -q fork lane/define-json-sync && git checkout -q --detach FETCH_HEAD && . /root/claude-opencode/lanes/env-define.sh && cd /root/claude-opencode/lanes/wt-define-json && $CMD"
