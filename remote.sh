...
# The remote shell has: cargo/rustc (nightly), LLVM 22, a warm shared CARGO_TARGET_DIR, $OPENCODE_SRC (OpenCode v1.18.30 with node_modules) and $OPENCODE_SRC/packages/opencode (resolve @babel/core from there).
...
git push -q -f fork "$commit:refs/heads/lane/babel-cjs-sync"
ssh -o BatchMode=yes root@perrymaster.skelpo.net "cd /root/claude-opencode/lanes/wt-babel-cjs && git fetch -q fork lane/babel-cjs-sync && git checkout -q --detach FETCH_HEAD && . /root/claude-opencode/lanes/env-babel.sh && cd /root/claude-opencode/lanes/wt-babel-cjs && $CMD"
