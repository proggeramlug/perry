...
git push -q -f fork "$commit:refs/heads/lane/cgu-str-sync"
ssh -o BatchMode=yes root@perrymaster.skelpo.net "cd /root/claude-opencode/lanes/wt-cgu-str && git fetch -q fork lane/cgu-str-sync && git checkout -q --detach FETCH_HEAD && . /root/claude-opencode/lanes/env.sh && cd /root/claude-opencode/lanes/wt-cgu-str && $CMD"
