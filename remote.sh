...
git push -q -f fork "$commit:refs/heads/lane/selfns-sync"
ssh -o BatchMode=yes root@perrymaster.skelpo.net "cd /root/claude-opencode/lanes/wt-selfns && git fetch -q fork lane/selfns-sync && git checkout -q --detach FETCH_HEAD && . /root/claude-opencode/lanes/env-selfns.sh && cd /root/claude-opencode/lanes/wt-selfns && $CMD"
