...
Fixed the construction cache remembering an incoherent program pair. The
...
never consults the maps again — so a lookbehind literal would have been born
permanently non-matching. The triple is now only remembered against the text
when it is coherent. (Reported by the regex lane, #9796 item 2, whose fix also
rebuilds the missing fallback.)
